...
import {
  getAnalysisNewsCob,
  getAnalysisTweetsCob,
} from "../../service/analysis";
import { getReview } from "../../service/review";
import { getFeedNewsCob } from "../../service/feed";
...
const Feed = () => {
  ...
  useEffect(() => {
    async function getAnaliseCobApi() {
      const response = await getAnalysisNewsCob();
      if (response) {
    ...
    async function getFeedApi() {
      const response = await getFeedNewsCob(`?page=${1}`);
      if (response) {
        setFeed({
    ...
    async function getTweetsAnaliseCobApi() {
      const response = await getAnalysisTweetsCob();
      if (response) {
    ...
    getAnaliseCobApi();
    getTweetsAnaliseCobApi();
  }, []);

  const handleInfinitScroll = async (type) => {
    if (type === "noticias") {
      const response = await getFeedNewsCob(`?page=${page.articles + 1}`);
      ...
      const response = await getFeedNewsCob(`?page=${page.tweets + 1}`);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-12 gap-3">
          <div className="lg:col-span-8 col-span-1 mt-10">
            {review ? (
              <AnalisesCard
                review={review}
                title={"Análise Metrô SP Perfil Oficial"}
              />
            ) : null}
