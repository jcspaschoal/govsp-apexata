...
import {
  getAnalysisNewsImprensa,
  getAnalysisTweetsImprensa,
} from "../../service/analysis";
import { getReview } from "../../service/review";
import { getFeedNewsImprensa } from "../../service/feed";
...
const Imprensa = () => {
  ...
  useEffect(() => {
    async function getAnaliseImprensaApi() {
      const response = await getAnalysisNewsImprensa();
      if (response) {
    ...
    async function getFeedApi() {
      const response = await getFeedNewsImprensa(`?page=${1}`);
      if (response) {
        setFeed({
    ...
    async function getAnalysisTweetsImprensasApi() {
      const response = await getAnalysisTweetsImprensa();
      if (response) {
    ...
    getAnaliseImprensaApi();
    getAnalysisTweetsImprensasApi();
  }, []);

  const handleInfinitScroll = async (type) => {
    if (type === "noticias") {
      const response = await getFeedNewsImprensa(`?page=${page.articles + 1}`);
      ...
      const response = await getFeedNewsImprensa(`?page=${page.tweets + 1}`);
      ...
      {feed ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-12 gap-3">
          <div className="lg:col-span-8 col-span-1 mt-10">
            {review ? (
              <AnalisesCard review={review} title={"Análise Imprensa"} />
            ) : null}
