...
import {
  getAnalysisNewsEsportes,
  getAnalysisTweetsEsportes,
} from "../../service/analysis";
import { getReview } from "../../service/review";
import { getFeedNewsEsportes } from "../../service/feed";
...
const Esportes = () => {
  ...
  useEffect(() => {
    async function getAnaliseEsportesApi() {
      const response = await getAnalysisNewsEsportes();
      if (response) {
    ...
    async function getFeedApi() {
      const response = await getFeedNewsEsportes(`?page=${1}`);
      if (response) {
        setFeed({
    ...
    async function getAnalysisTweetsEsportessApi() {
      const response = await getAnalysisTweetsEsportes();
      if (response) {
    ...
    getAnaliseEsportesApi();
    getAnalysisTweetsEsportessApi();
  }, []);

  const handleInfinitScroll = async (type) => {
    if (type === "noticias") {
      const response = await getFeedNewsEsportes(`?page=${page.articles + 1}`);
      ...
      const response = await getFeedNewsEsportes(`?page=${page.tweets + 1}`);
      ...
      {feed ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 lg:gap-12 gap-3">
          <div className="lg:col-span-8 col-span-1 mt-10">
            {review ? (
              <AnalisesCard review={review} title={"Análise Linhas Públicas"} />
            ) : null}
